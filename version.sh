...
# Get list of Cargo files from workspace Cargo.toml
CARGO_FILES=$(awk '/members = \[/{flag=1;next}/\]/{flag=0}flag{gsub("\""," ");print $1"/Cargo.toml"}' Cargo.toml)
# Extract name of packages from Cargo files
CARGO_LOCK_PROJECTS=$(grep "name =" $CARGO_FILES | awk '{gsub("\"", "");print $3}')
